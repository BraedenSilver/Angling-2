...
public class WormyMudBlock extends Block implements WormyBlock {
    public WormyMudBlock(BlockBehaviour.Properties props) {
        super(props);
    }
        ...
        if (random.nextInt(8) == 0) {
            ...
            for (int i = 1; i <= 5; i++) {
                ...
                if (!checkState.isRedstoneConductor(level, checkPos) || !checkState.isCollisionShapeFullBlock(level, checkPos)) {
                    ...
                    level.addParticle(AnglingParticles.WORM, x, y, z, 0, 0, 0);
                    break;
                }
            }
        }
