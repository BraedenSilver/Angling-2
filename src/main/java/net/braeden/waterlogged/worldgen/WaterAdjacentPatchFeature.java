...
public class WaterAdjacentPatchFeature extends Feature<NoneFeatureConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> ctx) {
        ...
        for (int attempt = 0; attempt < 64; attempt++) {
            ...
            if (!existing.canBeReplaced() && !existing.is(WaterloggedBlocks.ALGAE)) continue;

            // Find all directions with a sturdy neighboring face
            boolean isAlgaeAlready = existing.is(WaterloggedBlocks.ALGAE);
            BlockState algaeState = isAlgaeAlready
                    ? existing
                    : WaterloggedBlocks.ALGAE.defaultBlockState().setValue(AlgaeBlock.WATERLOGGED, true);
